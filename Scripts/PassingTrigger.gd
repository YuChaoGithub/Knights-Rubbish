...
func _process(delta):
	if in_range:
		...
		var success_y = y_check == NONE || (y_check == GREATER && char_average_pos.global_position.y > global_position.y) || (y_check == LESSER && char_average_pos.global_position.y < global_position.y)
