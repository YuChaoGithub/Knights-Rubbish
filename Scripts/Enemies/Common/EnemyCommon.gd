...
const HEALTH_MULTIPLIER_FOR_COOP = 1.5
...
func _init(node, default_status = 0):
    ...
    self.health_system = preload("res://Scripts/Utils/HealthSystem.gd").new(node, int(node.MAX_HEALTH * (HEALTH_MULTIPLIER_FOR_COOP if hero_manager.heroes.size() > 1 else 1)))
